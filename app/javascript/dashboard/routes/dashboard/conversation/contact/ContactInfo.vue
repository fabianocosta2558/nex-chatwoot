...
export default {
  ...
  computed: {
    ...
    businessAttributes() {
      return {
        ...(this.contact.custom_attributes || {}),
        ...(this.currentChat?.custom_attributes || {}),
      };
    },
    proposalBank() {
      return this.getBusinessField(['proposal_bank', 'bank', 'banco']);
    },
    proposalInstallment() {
      return this.formatCurrency(
        this.getBusinessField(['installment', 'parcela'])
      );
    },
    proposalTerm() {
      const value = this.getBusinessField(['offer_term', 'term', 'prazo']);
      if (value === '' || value === null || value === undefined) return '';
      const text = String(value).trim();
      return /x$/i.test(text) ? text : `${text}x`;
    },
    proposalReleasedAmount() {
      return this.formatCurrency(
    ...
    proposalJourneyUrl() {
      return this.getBusinessField([
        'journey_url',
        'link_url',
        'autocontratacao_url',
        'link_autocontratacao',
      ]);
    },
  },
  watch: {
  ...
  methods: {
    ...
    getBusinessField(keys) {
      for (const key of keys) {
        const value = this.businessAttributes[key];
    ...
    <div class="flex flex-col w-full gap-2 text-left rtl:text-right">
      ...
      <div class="flex flex-col items-start gap-1.5 min-w-0 w-full">
        ...
        <div class="flex flex-col items-start w-full gap-2">
          <ContactInfoRow
            :value="proposalBank ? `Banco: ${proposalBank}` : ''"
            icon="building-bank"
            emoji="🏦"
            title="Banco"
            show-copy
          />
          <ContactInfoRow
            :value="proposalInstallment ? `Parcela: ${proposalInstallment}` : ''"
          ...
          />
          <ContactInfoRow
            :value="proposalTerm ? `Prazo: ${proposalTerm}` : ''"
            icon="calendar"
            emoji="📅"
            title="Prazo"
            show-copy
          ...
          <ContactInfoRow
            :value="
              proposalReleasedAmount
                ? `Valor liberado: ${proposalReleasedAmount}`
                : ''
            "
            icon="credit-card-person"
            emoji="💰"
          ...
          />
          <ContactInfoRow
            :href="proposalJourneyUrl"
            :value="proposalJourneyUrl ? 'Abrir autocontratação' : ''"
            icon="link"
            emoji="🔗"
            title="Link de autocontratação"
            show-copy
          />
          <ContactInfoRow
